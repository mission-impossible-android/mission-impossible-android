...
def urlretrieve(url, output_filepath, cache_dir=None):
    """
    ...
    cmd = "wget --server-response --no-verbose --continue --output-document=" + output_filepath + " " + url
    args = shlex.split(cmd)
    child_process = subprocess.Popen(args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    matches = re.match(r'^ *HTTP\/[\d\.]+ (?P<code>\d{3}) (?P<msg>[\w ]*)$', raw_response_data)
    response_data = {
